...
OI::OI()
{
	...
	s_intake_belt_inward = new JoystickButton(joystick_buttons, OI_Ports::INTAKE_BELT_FORWARD_SWITCH);
	s_intake_belt_outward = new JoystickButton(joystick_buttons, OI_Ports::INTAKE_BELT_BACKWARD_SWITCH);
...
{
	return joystick_buttons->GetRawAxis(OI_Ports::FRONT_WINCH_JOYSTICK);
}

float OI::getBackWinchY()
{
	return joystick_buttons->GetRawAxis(OI_Ports::BACK_WINCH_JOYSTICK);
}

int OI::getShooterSpeedPosition()
{
	// assumes GetRawAxis returns in the range [0.0, 1.0]
	return Utils::voltageConversion(joystick_buttons->GetRawAxis(OI_Ports::SHOOTER_SPEED_DIAL), 6, 1.0);
}
